...
def _ensure_runtime_sitecustomize(runtime_channel: str) -> None:
    ...
    if not is_frozen() and uses_current_service_python(runtime_channel):
        ...
        return
    site_packages = runtime_site_packages_dir(runtime_channel)
    # Validate path is within managed runtime root
    site_packages = _assert_path_within_managed_runtime_root(site_packages, field_name="site-packages path")
...
def replace_runtime_with_base_copy(
    ...
    # Validate all paths are within managed runtime root to prevent path traversal
    target_dir = _assert_path_within_managed_runtime_root(target_dir, field_name="target runtime path")
    backup_dir = _assert_path_within_managed_runtime_root(backup_dir, field_name="backup runtime path")
    base_dir = _assert_path_within_managed_runtime_root(base_dir, field_name="base runtime path")
    temp_dir = _assert_path_within_managed_runtime_root(
        target_dir.parent / f".{runtime_channel}-refresh-temp",
        field_name="temporary runtime path",
    )
    if temp_dir.exists():
